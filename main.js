...
(() => {
  ...
  const SC = {
    ...
    game:{ label:'Make a small game', tag:'game', task:'Make me a small browser game',
      ...
      loop:{ Sense:'read the pitch · find the core loop before anything else',
        Classify:'domain(game) HIGH · planning med · tools low → tier <span class="hl">strong</span>',
  ...
  const DOMAINS = [
    ...
    { re:/(\bcopy\b|market this|position(ing)?|\blaunch\b|\bads?\b|\bseo\b|funnel|headline|slogan|outreach|\bpitch\b)/, dom:'go-to-market', lead:'mercatus', layers:['disciplina','mercatus'], tier:'strong', mach:'one agent', axes:A(0,1,2,3),
    ...
    { re:/(council|several models|panel of|ensemble|ask (multiple|many|other) models|cross-?model)/, dom:'cross-model council', lead:'concilium', layers:['concilium'], tier:'strong', mach:'council · gated', axes:A(0,1,3,2),
      strike:'N models answer blind → anonymized peer-review → a chairman synthesizes', prove:'the panel’s disagreement is <span class="g">shown, not hidden</span>', compound:'file the answer + why it won' },
